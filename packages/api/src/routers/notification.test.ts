...
describe("notificationRouter", () => {
  ...
  it("unreadCount returns count", async () => {
    ...
    // unreadCount uses select({ count: sql`count(*)::int` }).from().where()
    // where() is the terminal call — returns a promise with the count result
    ...
    mockDb.where.mockImplementation(() => {
      ...
      // Second where call: the count query — this is the terminal
  ...
  it("unreadCount filters by repoId when provided", async () => {
    ...
    const caller = createCaller(true);
    const result = await caller.notification.unreadCount({ repoId: REPO_ID });

    expect(result).toEqual({ count: 1 });
  });

  it("markAllRead scopes to repoId when provided", async () => {
    const REPO_ID = "880e8400-e29b-41d4-a716-446655440003";
    const updatedNotifs = [
      { ...mockNotification, id: "n1", read: true, repoId: REPO_ID },
    ];
    ...
    const caller = createCaller(true);
    const result = await caller.notification.markAllRead({ repoId: REPO_ID });

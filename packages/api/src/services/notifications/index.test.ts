  ...
  it("inserts 1 notification, publishes 1 sync event, looks up tokens, sends push", async () => {
    ...
    db.where.mockImplementation(() => {
      ...
      if (selectCallCount === 2) {
        // push tokens query
        return Promise.resolve([{ id: TOKEN_ID_1, userId: USER_ID_1, token: "push-token-abc", createdAt: new Date() }]);
      }
    ...
    expect(mockPush.sendBatch).toHaveBeenCalledWith([
      expect.objectContaining({ token: "push-token-abc", title: "Test", body: "Test notification" }),
    ]);
  });
